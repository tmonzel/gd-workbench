...
const INPUT_DIR = path.resolve("data/resources/items");
const OUTPUT_DIR = path.resolve("public/assets/items");
...
const findTexconv = () => {
  const toolPath = TEXCONV_CANDIDATES.find((candidate) =>
    fs.existsSync(candidate),
  );
  if (!toolPath)
    throw new Error(
      "texconv.exe not found. Set TEXCONV_PATH or place texconv.exe in the project root.",
    );
...
const getTexFiles = (directory) => {
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) files.push(...getTexFiles(entryPath));
      ...
      entry.isFile() &&
      path.extname(entry.name).toLowerCase() === ".tex"
    ) {
...
const getReferencedTexFiles = () => {
  if (!fs.existsSync(ITEMS_JSON)) return getTexFiles(INPUT_DIR);
...
const createDdsBuffer = (texBuffer, sourcePath) => {
  if (texBuffer.subarray(0, 4).toString("ascii") !== "TEX\x02") {
    throw new Error(
      `${sourcePath} does not have a supported Grim Dawn TEX header`,
    );
  }
  if (
    texBuffer.length <= 16 ||
    texBuffer.subarray(12, 16).toString("ascii") !== "DDSR"
  ) {
...
const readBitmapPixels = (texBuffer, sourcePath) => {
  ...
  if (pixels.length < expectedLength) {
    throw new Error(
      `${sourcePath} bitmap payload is incomplete: expected ${expectedLength}, got ${pixels.length}`,
    );
  }
...
const convertTexFile = async (sourcePath, texconvPath) => {
  const relativePath = path.relative(INPUT_DIR, path.dirname(sourcePath));
...
const main = async () => {
  ...
  for (const sourcePath of texFiles) {
    try {
      ...
      failed += 1;
      console.error(
        `[Failed] ${path.relative(process.cwd(), sourcePath)}: ${error.message}`,
      );
    }
  }
  fs.rmSync(TEMP_DDS_DIR, { recursive: true, force: true });
